...
const RhinoViewer = () => {
    ...
    useEffect(() => {
        ...
        const loadGeometry = async () => {
            if (!rhinoSolveRes || rhinoSolveRes == 'undefined' || rhinoSolveRes == '' || rhinoSolveRes == null) {
                try {
                    ...
                    // console.log(raw)
                    const raw = JSON.stringify({ "absolutetolerance": 0.01, "angletolerance": 1.0, "modelunits": "Meters", "dataversion": 7, "algo": null, "pointer": "md5_853B291B31B4B5DBDD794D45B640DE1C", "cachesolve": true, "values": [{ "ParamName": "Radius", "InnerTree": { "{0}": [{ "type": "System.Double", "data": "5.0" }] } }, { "ParamName": "Count", "InnerTree": { "{0}": [{ "type": "System.Double", "data": "10.0" }] } }, { "ParamName": "Length", "InnerTree": { "{0}": [{ "type": "System.Double", "data": "10.0" }] } }], "warnings": [], "errors": [] }
                    );
        ...
        const collectResults = async () => {
            ...
            // const buffer = await modelo3dm.json();
            // setRespuesta3(buffer.rhinoData);
            // console.log(typeof (respuesta3))
            // console.log(JSON.stringify(respuesta3))

            // console.log(respuesta3.byteLength);

            // loader.parse(respuesta3,
            //     function (object) {
            //         // Si la carga es exitosa, se añade el objeto a la escena
            //         scene.add(object);
            //     },
            //     function (error) {
            //         // Manejo de errores
            //         console.error('Error loading 3dm object:', error);
            //     }
            // );

            // loader.parse(respuesta3, function (object) {
            //     // debug 
    ...
    return <div ref={mountRef}>
        <div style={{ padding: "100px", }} >
            <p><b>Respuesta 1 (I/O):</b> {rhinoIoRes}</p>
            <p><b>Respuesta 2 (Grasshopper):</b> {rhinoSolveRes}</p>
            <p><b>Data Modelo:</b> {JSON.stringify(dataModelo)}</p>
            <p><b>Respuesta 3 (Archivo 3DM):</b> {JSON.stringify(respuesta3)}</p>
        </div>
